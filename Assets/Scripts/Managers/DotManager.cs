...
{
	public GameObject DotPrefab, SpherePrefab, LinePrefab;
	[HideInInspector]
	...
	void HandleTouchInput()
	{
		...
		if (Physics.Raycast(ray, out hit, 100f, spheresLayer))
		{
	...
	bool ValidateChainColor(Sphere s)
	{
		...
		else if (c == ChainColor || c == ArtManager.Instance.WHITEDOT_COLOR)
			result = true;
	...
	public IEnumerator Spawn(int rows, int columns, bool withPhysics)
	{
		...
		var spawnPoint = new Vector3(-1f * ArtManager.Instance.SCREEN_RATIO / 2f, _myTransform.position.y, _myTransform.position.z);
		...
		for (int j = 0; j < columns; j++)
		{
			for (int i = 0; i < rows; i++)
			{
				...
				GameObject sphere = Instantiate(SpherePrefab, dot.transform.position, Quaternion.identity) as GameObject;
				sphere.name = "Sphere";
				sphere.transform.parent = dot.transform;
				tempList.Add(dot);
				cleanList.Add(dot.GetComponentInChildren<Sphere>());
				_myTransform.position += new Vector3(0f, CIRCLE_SIZE / (float)rows * ArtManager.Instance.SCREEN_RATIO, 0f);
				if (withPhysics)
				{
					sphere.GetComponent<SphereCollider>().radius *= ArtManager.Instance.SCREEN_RATIO;
					dot.GetComponent<BoxCollider2D>().size *= ArtManager.Instance.SCREEN_RATIO;
					dot.rigidbody2D.gravityScale = 0f;
				}
			}
			_myTransform.position += new Vector3(CIRCLE_SIZE / (float)columns * ArtManager.Instance.SCREEN_RATIO, -((CIRCLE_SIZE / (float)rows) * (float)rows) * ArtManager.Instance.SCREEN_RATIO, 0f);
		}
		_myTransform.position = spawnPoint;
		...
		if (withPhysics)
		{
			...
			{
				dot.rigidbody2D.gravityScale = 1f;
				yield return null;
			}
		}
	}
