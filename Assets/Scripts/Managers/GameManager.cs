...
public class GameManager : MonoBehaviour
{
	...
	[HideInInspector]
	public int COLORSCORE_PER_POWER = 12;
	[HideInInspector]
	...
	void Awake()
	{
		...
		ACCEPTABLE_DISTANCE *= ArtManager.Instance.SCREEN_RATIO;
	...
	IEnumerator CoLoadNewGame()
	{
		...
		if ((DotManager.Instance.cleanList.Count + DotManager.Instance.lockedList.Count) > 0)
		{
			yield return StartCoroutine(DotManager.Instance.UnlockAllSpheres());
			yield return StartCoroutine(DotManager.Instance.HideAllSpheres());
			DotManager.Instance.DestroyAllSpheres();
		}
		yield return StartCoroutine(DotManager.Instance.Spawn(6, 6, true));
		yield return StartCoroutine(DotManager.Instance.LockSpheres());
		isRunningSomething = false;
	}
	...
	public void AddColorScore(Color32 c)
	{
		...
		if ((Color)c == (Color)ArtManager.Instance.GREYDOT_COLOR)
		{
			++GreyScore;
			return;
		}
		// If dot is white, add to all color scores.
		else if ((Color)c == (Color)ArtManager.Instance.WHITEDOT_COLOR)
		{
			++WhiteScore;
